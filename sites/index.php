  <div id="foryoupage">
    <?php
      $sql = "SELECT * FROM `userbild` ORDER BY RAND()";
      $abfrage = mysqli_query($verbindung, $sql);
      if(mysqli_num_rows($abfrage) > 0){
        $ubi = mysqli_fetch_assoc($abfrage);
        foreach($abfrage as $userbild){
          $bid = $userbild['BID'];
          $uid = $userbild['UID'];
          $error = false;
          $bild = "";
          $user = "";
          $sql2 = "SELECT * FROM bilder WHERE BID='{$bid}'";
          $abfrage = mysqli_query($verbindung, $sql2);
          if(mysqli_num_rows($abfrage) > 0){
          ...
          $abfrage = mysqli_query($verbindung, $sql3);
          if(mysqli_num_rows($abfrage) > 0){
            while($userbild = mysqli_fetch_assoc($abfrage)){
              $user = $userbild;
            }
          ...
          if($error == false) { ?>
            <div class="post">
              <a href="?p=profile&suid=<?php echo $user['UID']; ?>">
              ...
              <a href="?p=post&bid=<?php echo $bild['BID']; ?>">
              ...
              <?php if($bild['Beschreibung'] != null || $bild['Beschreibung'] != "") { ?>
                <p class="post-beschreibung"><span><?php echo $user['BName']; ?> </span> <?php echo $bild['Beschreibung']; ?></p>
              <?php } ?>
            </div>
            <?php
          }
